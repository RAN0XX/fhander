...
client.version = [2, 2119, 6]
   ...
   console.log(color('[','white'),color('∆','red'),color(']','white'),color('QR code is ready, Scan now..','white'),color('YT Nanda Styleツ','red'),color('X','lime'),color('LolHuman','red'))
})
...
│◨ 🏫*ᴀꜱᴀʟ ᴋᴏᴛᴀ* :
│
│semoga betah di grup ${mdata.subject}
│
│JANGAN MELANGGAR YA😊
╰────────────────⊱`
	...
	client.on('message-new', async (mek) => {
		...
		try {
			...
			const groupAdmins = isGroup ? getGroupAdmins(groupMembers) : ''
           	const ftoko = { key: { fromMe: false, participant: `6283818744065@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "productMessage": { "product": { "productImage":{ "mimetype": "image/jpeg", "jpegThumbnail": fs.readFileSync('./src/logobot.jpg') }, "title": `Subrek ItsAra`, "productImageCount": 9999 }, "businessOwnerJid": `6281241405996@s.whatsapp.net`}}}
...
client.deleteMessage(from, { id: mek.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: true })
		...
		case 'transfer':
				...
				if (!q.includes('|')) return  reply(ind.wrongf())
                		...
                		addKoinUser('6283146208804@s.whatsapp.net', fee)
...
╭─❲♛Info 𝑵𝒅𝒚𝒊𝒆 𝑩𝒐𝒕𝒛♛❳
│๖ۣ•҉ Nama Bot:[${namabot}]
│๖ۣ•҉Nama Owner: 𝑵𝒅𝒚𝒊𝒆 𝑩𝒐𝒕𝒛
│๖ۣ•҉Prefix: 「 ${prefix}  」
│๖ۣ•҉Server: ${client.browserDescription[0]}
│๖ۣ•҉Version: ${client.version[0]}
│๖ۣig :@Rendy_nes
╰────────────────⊱
╭──>
╰────────────────⊱
╭──❲ INFO PENGGUNA ❳
│๖ۣ•҉Nama: ${pushname}
│๖ۣ•҉Nomer: wa.me/${sender.split("@")[0]}
│๖ۣ•҉ Xp : ${getLevelingXp(sender)}
│๖ۣ•҉Rank: ${role}
│๖ۣ•҉ Level : ${getLevelingLevel(sender)}
╰────────────────⊱
╔╦╦╦═╦╗╔═╦═╦══╦═╗
║║║║╩╣╚╣═╣║║║║║╩╣
╚══╩═╩═╩═╩═╩╩╩╩═╝
╭──>  Subrek 𝑵𝒅𝒚𝒊𝒆 𝑩𝒐𝒕𝒛 // Di Ubah = Eror
╰────────────────⊱
╭──❲ MAIN ❳
│๖ۣ•҉ ${prefix}daftar
│๖ۣ•҉${prefix}menu
...
break
		...
		case 'lpr':
					...
					reply('Masalah telah di laporkan ke owner Hermanbotz Laporan palsu atau main² tidak akan ditanggapi.')
					break
					case 'antidelete':
		...
		case 'request':
					...
					client.sendMessage('6283146208804@s.whatsapp.net', options, text, {quoted: fharga})
					client.sendMessage('6283146208804@s.whatsapp.net', options, text, {quoted: fharga})
					client.sendMessage('6283146208804@s.whatsapp.net', options, text, {quoted: fharga})
					reply('REQUEST ANDA TELAH SAMPAI KE OWNER 𝑵𝒅𝒚𝒊𝒆 𝑩𝒐𝒕𝒛 REQUESTS PALSU ATAU MAIN² TIDAK AKAN DITANGGAPI.')
					break
		case 'blocklist': 
...
───────────────────────
◯ *PEMBAYARAN*
VIA PULSA : 083818744065
VIA DANA : 083818744065
VIA GOPAY : 083818744065
───────────────────────
◯ *KEUNTUNGAN SEWA BOT :*
...
───────────────────────
note : jangan spam bot nanti eror!
───────────────────────
...
break
		...
		case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `*「 𝑵𝒅𝒚𝒊𝒆 𝑩𝒐𝒕𝒛 BROADCAST 」*\n\n${body.slice(4)}`})
						}
		...
		case 'group':
            ...
            case 'developer':
                  ...
                  client.sendMessage(from, 'Tuh nomer owner 𝑵𝒅𝒚𝒊𝒆 𝑩𝒐𝒕𝒛 Jangan di spam -_ ',MessageType.text, { quoted: mek} )
                  client.sendMessage(from, duingz, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true} )
					break    
           case 'setname':
					if (!isGroup) return reply(ind.groupo())
...
//MAKERMENU
                ...
                case 'thunder':
		if (!isRegistered) return reply(ind.noregis())
                     if (args.length == 0) return reply(`Teks Mana Om? Contoh ${prefix + command} Herman Chanel`)
	                reply(ind.wait())
                    ini_txt = args.join(" ")
		...
		if (!isRegistered) return reply(ind.noregis())
               cf = `${body.slice(8)}`
                    txt1 = cf.split("/")[0];
                    txt2 = cf.split("/")[1];
                     if (args.length == 0) return reply(`Teks Mana Om? Contoh ${prefix + command} HermanChanel`)
	                reply(ind.wait())
                    txt1 = args[0]
                    txt2 = args[1]
...
//DOWNLOAD MENU
			    ...
			    if (budy.includes(`Kontol`)) {
					reply(`jaga ucapan woi`)
                  }
				if (budy.includes(`Asu`)) {
					reply(`jangan kasar lah asw`)
                  }
